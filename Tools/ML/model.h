...
class OnnxModel
{
 ...
 public:
                         ...
                         const std::string& maskInput = "")
  {
    ...
    // --- the single [N, K] graph input of the original model. We reproduce it by
    //     concatenating the K preprocessed feature columns and then copy the whole
    //     original network (first layer included) verbatim on top of it. Feeding a
    //     single Gemm keeps only one [M, K] buffer live, unlike a per-column
    //     decomposition which would materialise K partial [M, H] tensors at once. ---
    if (og.input_size() < 1) {
      LOG(fatal) << "setupColumnInputs: model has no graph input";
      return;
    }
    const std::string modelInputName = og.input(0).name();
    const int K = numFeatures;
    ...
    // --- if a mask is provided, Compress every per-track input to the selected
    //     rows ONCE up front, so all preprocessing + the matmul operate on the M
    //     valid rows instead of N (keeps the ONNX scratch footprint to the subset
    //     and the output compact). Per-collision arrays and scalars pass through. ---
    std::map<std::string, std::string> compressed;
    if (!maskInput.empty()) {
      const int64_t axis0 = 0;
      for (const auto& pin : inputDefs) {
        const bool perTrack = pin.type == PreprocInput::Type::TrackFloat ||
                              pin.type == PreprocInput::Type::TrackInt32 ||
                              pin.type == PreprocInput::Type::TrackUint8 ||
                              pin.type == PreprocInput::Type::TrackInt8;
        if (!perTrack || pin.name == maskInput) {
          continue;
        }
        std::vector<Ort::OpAttr> ca;
        ca.emplace_back("axis", &axis0, 1, ORT_OP_ATTR_INT);
        const std::string out = pin.name + "_c";
        Ort::Node compressNode("Compress", "", "_compress_" + pin.name,
                               {pin.name, maskInput}, {out}, ca);
        graph.AddNode(compressNode);
        compressed[pin.name] = out;
      }
    }
    auto resolve = [&](const std::string& n) -> std::string {
      auto it = compressed.find(n);
      return it == compressed.end() ? n : it->second;
    };

    // --- preprocessing subgraph: produce one [M] float feature tensor per column ---
    ...
    for (int i = 0; i < K; ++i) {
      ...
      switch (f.op) {
        case PreprocFeature::Op::Passthrough: {
          featNames[i] = resolve(f.a); // the (compressed) raw input is already the feature
          break;
        }
        case PreprocFeature::Op::BroadcastScalar: {
          Ort::Node shapeNode("Shape", "", p + "_shape", {resolve(f.shapeRef)}, {p + "_shp"});
          graph.AddNode(shapeNode);
          Ort::Node expandNode("Expand", "", p + "_expand", {resolve(f.a), p + "_shp"}, {p});
        ...
        case PreprocFeature::Op::NClsSqrtRecip: {
          ...
          Ort::Node castA("Cast", "", p + "_castA", {resolve(f.a)}, {p + "_a"}, ca);
          graph.AddNode(castA);
          std::vector<Ort::OpAttr> cb;
          cb.emplace_back("to", &toFloat, 1, ORT_OP_ATTR_INT);
          Ort::Node castB("Cast", "", p + "_castB", {resolve(f.b)}, {p + "_b"}, cb);
          graph.AddNode(castB);
        ...
        case PreprocFeature::Op::Mod2: {
          ...
          Ort::Node mod1("Mod", "", p + "_mod1", {resolve(f.a), p + "_c0"}, {p + "_m1"}, m1a);
          graph.AddNode(mod1);
        ...
        case PreprocFeature::Op::GatherNormWhere: {
          ...
          Ort::Node gatherNode("Gather", "", p + "_gather", {resolve(f.a), resolve(f.b)}, {p + "_g"});
          ...
          graph.AddNode(divNode);
          Ort::Node lessNode("Less", "", p + "_less", {resolve(f.b), "_decZeroI32"}, {p + "_amb"});
    ...
    // --- concatenate the K preprocessed [M] feature columns into the [M, K]
    //     tensor the original model expects, reproducing its graph input. Inputs
    //     were already Compress'd up front, so each feature is the M-row tensor. ---
    std::vector<std::string> unsqueezed(K);
    for (int i = 0; i < K; ++i) {
      unsqueezed[i] = "_col_unsq_" + std::to_string(i);
      Ort::Node unsqNode("Unsqueeze", "", "_unsqueeze_" + std::to_string(i),
                         {featNames[i], "_col_axes"}, {unsqueezed[i]});
      graph.AddNode(unsqNode);
    }
    {
      const int64_t axis1 = 1;
      std::vector<Ort::OpAttr> ca;
      ca.emplace_back("axis", &axis1, 1, ORT_OP_ATTR_INT);
      Ort::Node concatNode("Concat", "", "_concat_features", unsqueezed, {modelInputName}, ca);
      graph.AddNode(concatNode);
    }

    // --- copy the whole original network (first layer included): every node
    //     reachable backwards from the graph outputs. They consume the Concat
    //     output through the original graph-input name. ---
    ...
    LOG(info) << "setupColumnInputs: rebuilt model with " << rawInputNames.size() << " raw inputs -> "
              << K << " preprocessed features concatenated into the original first layer, "
              << copiedNodes << " nodes copied";
  }
